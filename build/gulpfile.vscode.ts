...
import { getMxcExcludeFilter, getRipgrepExcludeFilter } from './lib/platformPackages.ts';
...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		// Community has its own display/release version. Keep the packaged
		// package.json/product.version on the exact upstream API version so
		// built-in extension engine ranges do not treat it as a prerelease.
		if (quality && quality !== 'stable' && quality !== 'community') {
			version += '-' + quality;
		}
		...
		if (platform === 'win32') {
			all = es.merge(all, gulp.src([
				...
				'resources/win32/code-community.png'
			], { base: '.' }));
...
function patchWin32DependenciesTask(destinationFolderName: string) {
	...
	return async () => {
		...
		const deps = [product.nameShort + '.exe', ...(await Promise.all([
		...
		const packagedProduct = JSON.parse(await fs.promises.readFile(path.join(cwd, versionedResourcesFolder, 'resources', 'app', 'product.json'), 'utf8'));
		...
		const patchPromises = deps.map<Promise<unknown>>(async dep => {
			...
			await rcedit(fullPath, {
				...
				'version-string': {
					...
					'FileDescription': packagedProduct.nameLong,
					'FileVersion': packageJson.version,
					'InternalName': basename,
					'LegalCopyright': 'Copyright (C) 2026 PCL-N Edition Contributors',
					'OriginalFilename': basename,
					'ProductName': packagedProduct.nameLong,
					'ProductVersion': packagedProduct.communityRelease ?? packageJson.version,
				}
			});
		});
