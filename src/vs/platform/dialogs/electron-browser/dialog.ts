...
export function createNativeAboutDialogDetails(productService: IProductService, osProps: IOSProperties): { title: string; details: string; detailsToCopy: string } {
	const upstreamVersion = productService.upstreamVersion ?? productService.version;
	let version = productService.communityRelease ?? productService.version;
	...
	const getDetails = (useAgo: boolean): string => {
		if (productService.communityRelease) {
			return localize({ key: 'aboutDetailCommunity', comment: ['Electron, Chromium, Node.js and V8 are product names that need no translation'] },
				"Version: {0}\nUpstream Version: {1}\nCommit: {2}\nDate: {3}\nElectron: {4}\nElectronBuildId: {5}\nChromium: {6}\nNode.js: {7}\nV8: {8}\nOS: {9}",
				version,
				upstreamVersion,
